...
export default function CalendarGrid({ currentDate, events, onDayClick, selectedDate }) {
  ...
  return (
    <div className="calendar-grid">
      ...
      {cells.map((day, idx) => {
        if (day === null) return <div className="calendar-cell-premium empty" key={`empty-${idx}`} />;
        ...
        return (
          <motion.div
            className={`calendar-cell-premium ${isToday ? 'today' : ''} ${isSelected ? 'selected' : ''}`}
            key={dateStr}
            ...
            <div className="cell-dots">
              {dayEvents.slice(0, 4).map((e) => (
                <span
                  key={e.id}
                  className="event-dot"
                  style={{ background: typeColors[e.type], borderColor: 'var(--color-surface)' }}
                  title={e.title}
                />
              ))}
